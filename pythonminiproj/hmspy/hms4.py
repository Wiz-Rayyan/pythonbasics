...
def connect_to_mysql():
    global mycon, cursor, userName, password
    def try_connect():
        nonlocal username_entry, password_entry, login_window
    ...
    Label(login_window, text="MySQL Username:").pack(pady=5)
    username_entry = Entry(login_window)
    username_entry.pack(pady=5)

    Label(login_window, text="MySQL Password:").pack(pady=5)
    password_entry = Entry(login_window, show="*")
    password_entry.pack(pady=5)

    Button(login_window, text="Connect", command=try_connect).pack(pady=10)
...
def enter_booking_details():
    ...
    def save_details():
        ...
        create_table = """CREATE TABLE IF NOT EXISTS BOOKINGS(
            NAME VARCHAR(30), PHONE VARCHAR(20), ROOM_TYPE VARCHAR(20), NIGHTS INT)"""
        cursor.execute(create_table)
        sql = "INSERT INTO BOOKINGS VALUES(%s, %s, %s, %s)"
        values = (booking_details['name'], booking_details['phone'], booking_details['room_type'], booking_details['nights'])
...
def display_guest_details():
    connect_to_db()
    cursor.execute("SELECT * FROM BOOKINGS ORDER BY NAME DESC LIMIT 1")
    row = cursor.fetchone()
    if row:
        details = f"Name: {row[0]}\nPhone: {row[1]}\nRoom: {row[2]}\nNights: {row[3]}"
        messagebox.showinfo("Guest Details", details)
    else:
        messagebox.showwarning("Missing Info", "No booking details found in database.")
...
options = [
    ("0 | Connect to MySQL", connect_to_mysql),
